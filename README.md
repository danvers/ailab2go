...
|---|---|---|
| 🔍 **Objekt-Detektiv** | Live YOLO object detection with a confidence slider | Models pattern-match, they don't "understand"; the precision/recall trade-off |
| 🧠 **Trainiere die KI** | Train a classifier on any two objects in ~1 minute | Learning from examples, bias from one-sided data, right to erasure (one-click delete) |
| 🛡️ **Privatsphäre-Schild** | Faces get pixelated/blurred *at the source*; toggle it, break it | Detection ≠ recognition; tech can protect privacy; no shield is perfect |
